...
void Algorithm::aStar(Node** map1, int width, int height, Play::Vector2f start, Play::Vector2f goal, long long* runtime)
{
	if (found)
	{
		return;
	...
	while (!found)
	{
		...
		if (opened[smallestID]->GetPosition() == goal)
		{
			...
			auto ms = duration_cast<microseconds>(stopTime - startTime).count();
			*runtime = ms;
			found = true;
		}
		else
...
void Algorithm::Breadth(Node** map1, int width, int height, Play::Vector2f start, Play::Vector2f goal, long long* runtime)
{
	if (addStart)
	{
		int x = start.x / 20;
...
void Algorithm::Depth(Node** map1, int width, int height, Play::Vector2f start, Play::Vector2f goal, long long* runtime)
{
	if (addStart)
	{
		int x = start.x / 20;
...
void Algorithm::Dijkstra(Node** map1, int width, int height, Play::Vector2f start, Play::Vector2f goal, long long* runtime)
{
	if (found)
	{
		return;
